...
@Component({
  ...
  templateUrl: './jsr.component.html',
  styleUrl: './jsr.component.scss',
})
export class JsrComponent {
  // Data signals
  protected readonly data = signal<JsrItem[]>(mockJsrData);
  protected readonly columns = signal(jsrColumns);
  ...
  protected readonly filteredData = computed(() => {
    ...
    return this.data().filter((item) => {
      return (
        item.taskNum?.toLowerCase().includes(query) ||
        item.taskNote?.toLowerCase().includes(query) ||
        item.taskRa?.toLowerCase().includes(query) ||
        item.taskJob?.toLowerCase().includes(query) ||
        item.specItem?.toLowerCase().includes(query) ||
        item.mod?.toLowerCase().includes(query) ||
        item.clin?.toLowerCase().includes(query)
      );
    });
  });
